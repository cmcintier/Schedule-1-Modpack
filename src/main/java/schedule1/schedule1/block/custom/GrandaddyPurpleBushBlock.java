        ...
        if(!bl && stack.isOf(Items.BONE_MEAL)){
            ...
            if (!world.isClient) {
                ...
                if (world.getBlockState(pos).get(HARVESTED).equals(state.get(MAX_YIELD))) {
                    //Resets blockstates
                    BlockState blockStateAge = world.getBlockState(pos).with(AGE, 0);
                    world.setBlockState(pos, blockStateAge, Block.NOTIFY_LISTENERS);
                    world.setBlockState(pos, world.getBlockState(pos).with(HARVESTED, 0));
                    world.setBlockState(pos, world.getBlockState(pos).with(CURRENT_YIELD, 0));
                    world.emitGameEvent(GameEvent.BLOCK_CHANGE, pos, GameEvent.Emitter.of(player, blockStateAge));
                }
            }
